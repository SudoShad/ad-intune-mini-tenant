...
|---------------------|-----------|-----------------|
| Proxmox VMs | M365 Business Premium trial tenant (Intune) | Full Autopilot production |
...
1. M365 tenant with Intune is live (Business Premium trial after Dev sandbox block); Intune admin center reachable.
...
- [x] Attempted M365 Developer Program — **BLOCKED** (no E5 sandbox eligibility)
- [ ] **Pivot:** Microsoft 365 Business Premium **30-day free trial** (includes Intune). Card may be required for verify; **cancel before day 30**. Do not buy Enterprise E3/E5.
- [ ] Confirm tenant at https://admin.microsoft.com (Global Admin on trial tenant)
- [ ] Confirm Intune at https://intune.microsoft.com
- [ ] On Proxmox: clone or create WIN11-ENDPOINT (4–8 GB RAM); snapshot `clean`
- [x] GitHub repo `ad-intune-mini-tenant` created; docs live under `docs/`
- [ ] Reply to Sin: `trial ready` + Proxmox VM name + Win10/11

### Phase 1 — Entra + Intune (Nights 1–2) ← must publish
- [ ] Create `lab.endpoint@<tenant>.onmicrosoft.com` (or similar); assign **Business Premium** (Intune) license
...
| Activity | You | Sin |
|----------|-----|-----|
| Proxmox / Windows / portal clicks Sin names | **Execute only** | **Lead — name exact next click** |
...
**Kill criteria:** Dev sandbox already blocked — pivot to Business Premium trial is active. If trial signup is refused or requires immediate paid purchase with no free month, Sin calls the next pivot (education tenant or AD+GPO deepening only). You don’t invent a third stack.
...
| Risk | Mitigation |
|------|------------|
| Dev Program sandbox ineligible | **Done:** pivoted to Business Premium 30-day trial; cancel before paid |
| Trial auto-renews to paid | Sin reminder Oct 15; cancel in Billing |
| Intune license missing on user | Assign Business Premium to lab user before enroll |
...
1. Business Premium trial: `trial ready` or paste exact blocker screen  
2. Proxmox WIN VM name  
